...
export default function GestorDashboard() {
  ...
  // Sincronizar status do termo da ZapSign
  const handleSyncTermo = async (id: string) => {
    setSyncingId(id);
    try {
      const res = await axios.get(`/api/webhooks/zapsign?cooperadoId=${encodeURIComponent(id)}`);
      if (res.data?.status === 'Assinado' || res.data?.success) {
        await fetchData();
      }
    } catch (err) {
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {!isApproved && (
                            <>
                              ...
                              {/* Sincronizar Assinatura ZapSign */}
                              {isWaiting && (
                                <button
                                  onClick={() => handleSyncTermo(coop._id)}
                                  disabled={syncingId === coop._id}
                                  className="bg-amber-50 border border-amber-200 hover:bg-amber-100 text-amber-700 px-2.5 py-2 rounded-lg text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-1 shadow-sm"
                                  title="Verificar se o termo já foi assinado na ZapSign"
                                >
                                  {syncingId === coop._id ? (
                                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                  ) : (
                    ...
                    {selectedCooperado.txt_termo_status && (
                      <div className="bg-indigo-50/50 border border-indigo-100 p-2.5 rounded-lg flex items-center justify-between text-xs mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          {selectedCooperado.txt_termo_status === 'Aguardando Assinatura' && (
                            <button
                              onClick={() => handleSyncTermo(selectedCooperado._id)}
                              disabled={syncingId === selectedCooperado._id}
                              className="bg-amber-100 hover:bg-amber-200 text-amber-800 px-2 py-0.5 rounded text-[10px] font-bold transition-all flex items-center gap-1"
                              title="Sincronizar assinatura da ZapSign"
                            >
                              {syncingId === selectedCooperado._id ? (
